...
4. Run the **API** cell: it calls `GET /api/datasets/logs/summary` and `POST /api/train` on `http://127.0.0.1:8000` inside the same Colab session (`GET /api/datasets/summary` is also supported as an alias).
